...
# Einlesen der Daten.
us_macro <- read.table(here("07-session-01-07-trends", "01-daten", "us_macro_quarterly_merged.csv"),
                       header = TRUE,
                       sep = ";"
)
...
# Berechnung log GDP log GDP Differenz
LGDP <- log(us_macro_ts[,"GDPC1"])
LGDPDIF <- log(us_macro_ts[,"GDPC1"]) - log(stats::lag(us_macro_ts[,"GDPC1"], -1))

us_macro_ts <- cbind(us_macro_ts, GDPGR, TSpread, LGDP, LGDPDIF)
