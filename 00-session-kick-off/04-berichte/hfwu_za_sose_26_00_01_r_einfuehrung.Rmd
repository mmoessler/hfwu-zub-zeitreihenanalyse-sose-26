...
```{r simulation, fig.cap="Simulierte Daten"}
set.seed(90210)                         
x <- rnorm(25, 10, 4)                   
...
```{r regression, fig.cap="Beobachtungen und geschätzte Regressionsgerade"}
set.seed(1999)          
(x <- rnorm(10))        
(y <- x + rnorm(10))    
summary(fit <- lm(y~x)) 
...
```{r speech, fig.cap="Eine Zeitreihe in einem Plot"}
par(mfrow = c(1,1))
plot(speech)
```
...
```{r plots, fig.cap="Mehrere Zeitreihen in mehreren Plots"}
...
```{r ts_plot, fig.cap="Mehrere Zeitreihen in einem Plot"}
par(mfrow = c(1,1))
